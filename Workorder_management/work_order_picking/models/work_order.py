...
class WorkOrder(models.Model):
    ...
    def _get_move_value(self):
        ...
        move_value = []
        for line in lines:
            description_picking = line.product_id.with_context(lang=self.project_id.partner_id.lang or self.env.user.lang)._get_description(self.picking_type)
            moves = (0, 0, {
...
class ProductLine(models.Model):
    ...
    move_ids = fields.One2many('stock.move', 'product_line_id', string='Reservation', readonly=True, copy=False)
    ...
    def _compute_qty_done(self):
        for line in self:
            ...
            if len(line.move_ids) > 0:
                qty = sum([move.quantity_done for move in line.move_ids])
            line.qty_done = qty
