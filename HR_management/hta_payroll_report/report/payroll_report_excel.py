...
class PaybookReport(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        bottom_row_style = workbook.add_format({'bold': True, 'align': 'center', 'border': True, 'num_format':'# ##0'})
        left_row_style = workbook.add_format({'bold': False, 'align': 'left', 'border': True})
        cel_row_style = workbook.add_format({'bold': False, 'align': 'center', 'border': True, 'num_format':'# ##0'})
        cel_row = workbook.add_format({'bold': False, 'align': 'center', 'border': True})
        ...
        for rule in rules:
            ...
            worksheet.write_number(row_1, col, TOTAL, bottom_row_style)
            col += 1    
