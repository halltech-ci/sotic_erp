...
class HrAttendanceSheet(models.Model):
    ...
    state = fields.Selection(
        [
            ...
            ("done", "Approved"),
            ("locked", "Locked"),
        ], default="draft", tracking=True, string="Status", required=True, readonly=True, index=True,)
    ...
    working_hours = fields.Many2one("resource.calendar", string="Working Hours", related="employee_id.resource_calendar_id",)
    hours_to_work = fields.Float(related="employee_id.hours_to_work", string="Hours to Work", help="""Expected working hours based on company policy. This is used on attendance sheets to calculate overtime values.""",)
    total_time = fields.Float(compute="_compute_total_time", store=True)
    overtime = fields.Float(compute="_compute_overtime", store=True)
    can_review = fields.Boolean(string="Can Review", compute="_compute_can_review", search="_search_can_review")
    reviewer_id = fields.Many2one("hr.employee", string="Reviewer", readonly=True, tracking=True)
    reviewed_on = fields.Datetime(string="Reviewed On", readonly=True)
    review_policy = fields.Selection(string="Review Policy", related="company_id.attendance_sheet_review_policy")
